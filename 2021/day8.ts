...
const outputs = chunked.reduce((numAgg, [input, output]) => {
    ...
    function getNineAndSix(length6Chars: string[][], positions: StringToString, uniqueDigits: StringToString) {
        const sixAndNine = length6Chars.filter(chars => chars.includes(positions.middle))
        const [six, nine] = sixAndNine.reduce((acc, chars) => {
            const isNine = uniqueDigits[1].split("").every(char => chars.includes(char));
            if (isNine) {
                acc[1] = chars.join('');
            } else {
                acc[0] = chars.join('');
    ...
    mapping = { ...mapping, ...getNineAndSix(length6Chars, positions, uniqueDigits) };

    // Out of 0, 6 and 9, 0 is the one without the middle
    function getZero(length6chars: string[][], positions: StringToString) {
        const zero = length6chars.filter(chars => !chars.includes(positions.middle))[0].join('')
        return { [zero]: 0 }
    }
    ...
    function getThree(length5Chars: string[][], uniqueDigits: StringToString) {
        const oneChars = uniqueDigits[1].split('');
        const three = length5Chars.find(chars => oneChars.every(char => chars.includes(char)))!.join('')
        return { [three]: 3 }
    }
    mapping = { ...mapping, ...getThree(length5Chars, uniqueDigits) }


    const three = Object.entries(mapping).find(([_key, value]) => value === 3)![0];
    // Out of 5 and 2, only 5 has the top left
    function getFiveAndTwo(length5Chars: string[][], three: string, positions: StringToString) {
    ...
    const uniqueLengthChars = Object.fromEntries(Object.entries(uniqueDigits).map(([key, value]) => [value, Number(key)]))
    mapping = { ...mapping, ...uniqueLengthChars }
    
    mapping = Object.fromEntries(Object.entries(mapping).map(([key, value]) => [key.split('').sort().join(''), value]))
    const result = output.map(digit => mapping[digit.split('').sort().join('')]);
    numAgg.push(result);
    return numAgg
}, [] as number[][])

const nums = outputs.map(item => Number(item.join('')))
console.log('Part 2: ', nums.reduce((acc, num) => acc + num, 0))